...
public class SubFragmentMenWomenSports extends PointsFragment {
    ...
    public RecyclerView.Adapter setAdapter(ArrayList arrayList, Context context) {
        return new AdapterMenWomenSports(arrayList,context);
    }

    public static Sport getCurrentSport() {
        return SubFragmentMenWomenSports.currentSport;
    }

    public static void setCurrentSport(Sport currentSport) {
        SubFragmentMenWomenSports.currentSport = currentSport;
    }

    public void updateCurrentFragment(){
        if(getCurrentSport()==null) return;
        new RetrieveFeedTask().execute(API+ FragmentRanking.getCurrentYear()+"/sport/"+ getCurrentSport().getId());
    }
    ...
    class RetrieveFeedTask extends AsyncTask<String, Void, String> {
        ...
        public void saveJsonData(String jsonData){
            switch (current){
                case MEN:
                    FragmentRanking.saveJsonData(context, currentSport.getName()+"-Men-"+FragmentRanking.getCurrentYear(),jsonData);
                    break;
                case WOMEN:
                    FragmentRanking.saveJsonData(context, currentSport.getName()+"-Women-"+FragmentRanking.getCurrentYear(),jsonData);
            }
        }
        public String getJsonData(){
            switch (current){
                case MEN:
                    return FragmentRanking.getJsonData(context, currentSport.getName()+"-Men-"+FragmentRanking.getCurrentYear());
                case WOMEN:
                    return FragmentRanking.getJsonData(context, currentSport.getName()+"-Women-"+FragmentRanking.getCurrentYear());
            }
            return null;
        }
